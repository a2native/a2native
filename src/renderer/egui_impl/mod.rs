    ...
    match lang.as_str() {
        "zh" => {
            ...
            if matches!(sub.as_str(), "TW" | "HK" | "MO" | "HANT") {
                "⚠  此介面由 AI 代理程式生成，除非您信任該來源，否則請勿輸入敏感資訊。"
            } else {
                "⚠  此界面由 AI 代理生成，除非您信任该来源，否则请勿输入敏感信息。"
            }
        }
        "ja" => "⚠  このインターフェースはAIエージェントが生成しました。信頼できる場合以外は機密情報を入力しないでください。",
        "ko" => "⚠  이 인터페이스는 AI 에이전트가 생성했습니다. 신뢰할 수 없는 경우 민감한 정보를 입력하지 마세요。",
        "fr" => "⚠  Cette interface a été générée par un agent IA. Ne saisissez pas d'informations sensibles à moins de faire confiance à la source.",
        "de" => "⚠  Diese Oberfläche wurde von einem KI-Agenten generiert. Geben Sie keine vertraulichen Daten ein, sofern Sie der Quelle nicht vertrauen.",
        "es" => "⚠  Esta interfaz fue generada por un agente de IA. No introduzca información sensible a menos que confíe en la fuente.",
        "pt" => "⚠  Esta interface foi gerada por um agente de IA. Não insira informações sensíveis a menos que confie na fonte.",
        "it" => "⚠  Questa interfaccia è stata generata da un agente IA. Non inserire informazioni sensibili a meno che tu non ti fidi della fonte.",
        "ru" => "⚠  Этот интерфейс создан агентом ИИ. Не вводите конфиденциальные данные, если не доверяете источнику.",
        "ar" => "⚠  تم إنشاء هذه الواجهة بواسطة وكيل ذكاء اصطناعي — لا تُدخل معلومات حساسة ما لم تثق بالمصدر.",
        "hi" => "⚠  यह इंटरफ़ेस एक AI एजेंट द्वारा बनाया गया है — जब तक आप स्रोत पर भरोसा न करें, संवेदनशील जानकारी न डालें।",
        "tr" => "⚠  Bu arayüz bir yapay zeka ajanı tarafından oluşturuldu. Kaynağa güvenmiyorsanız hassas bilgi girmeyin.",
        "nl" => "⚠  Deze interface is gegenereerd door een AI-agent. Voer geen gevoelige informatie in tenzij u de bron vertrouwt.",
        "pl" => "⚠  Ten interfejs został wygenerowany przez agenta AI. Nie wprowadzaj poufnych danych, jeśli nie ufasz źródłu.",
        "vi" => "⚠  Giao diện này được tạo bởi tác nhân AI. Không nhập thông tin nhạy cảm trừ khi bạn tin tưởng nguồn.",
        "th" => "⚠  อินเทอร์เฟซนี้สร้างโดย AI agent — อย่าป้อนข้อมูลที่ละเอียดอ่อนหากคุณไม่ไว้วางใจแหล่งที่มา",
        _ => "⚠  This interface was generated by an AI agent — do not enter sensitive information unless you trust the source.",
    }
}
